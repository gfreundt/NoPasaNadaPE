from src.updates.get_recipients import need_alert, need_message


def get_records(db_cursor):

    need_alert(db_cursor)
    need_message(db_cursor)
    ...
    updates["satimps"] = get_records_satimps(db_cursor, HLA=RECENTLY)
    ...
    return {i: list(set(j)) for i, j in updates.items()}
