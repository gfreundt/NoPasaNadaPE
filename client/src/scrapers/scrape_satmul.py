...
def browser(placa):
    ...
    while xpath(n, 1):
        ...
        # image found, grab it and decode it to a string
        else:

            # navigate to page with image
            webdriver.get(_imgs[0].get_attribute("href"))
            time.sleep(0.5)
        ...
        resp.update({"img_binary": str(os.path.basename(img_filename))})
